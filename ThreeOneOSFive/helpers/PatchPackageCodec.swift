...
enum PatchPackageCodec {
    ...
    // Built-in resources are encrypted at rest. These values are only used by
    // the application to unlock bundled resources; imported packages still
    // require the user-supplied password. Keep the legacy value for existing
    // xTop1 packages and support XRE for the new ATH packages.
    static let bundledResourcePassword = "xTop1-External-Internal-Patch-Key-2026"
    static let bundledResourcePasswords = [bundledResourcePassword, "XRE"]
