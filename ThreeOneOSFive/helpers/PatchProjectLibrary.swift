...
enum PatchProjectLibrary {
    ...
    static func installBundledPackagesIfNeeded(
        ...
        // Xcode may flatten resources or place them in a nested folder. Walk
        // the complete bundle so packages are found in either layout.
        ...
        var seen = Set<String>()
        let bundledURLs = discoveredURLs.filter { seen.insert($0.standardizedFileURL.path).inserted }
    ...
    static func load(fileManager: FileManager = .default) -> [PatchLibraryItem] {
        ...
        // Keep entries distinct by package URL, not package UUID. Bundled
        // variants may intentionally share the same package ID/content key
        // while being exposed as separate UI buttons (for example NORMAL and
        // MAX filenames). UUID-based deduplication would make the other
        // buttons report "PACKAGE NOT FOUND".
        var byURL: [String: PatchLibraryItem] = [:]
        for url in urls where url.pathExtension.lowercased() == "3105" {
            do {
                ...
                if let contentKey = try PatchKeyStore.load(for: summary) {
                    ...
                    // Only bundled resources use the internal keys; imported
                    // packages remain locked and continue to require the user
                    // password. This keeps the password out of the UI flow.
                    let filename = url.deletingPathExtension().lastPathComponent
                    let isBundledResource = filename.hasPrefix("xTop1 External File (")
                        || filename.hasPrefix("ATH ")
                ...
                byURL[url.standardizedFileURL.path] = item
            } catch {
