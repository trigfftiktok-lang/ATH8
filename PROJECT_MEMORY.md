...
- Package inspection confirmed all six supplied ATH packages contain the target bundle ID `com.dts.freefireth`. `ContentView` now adapts that target at apply time to `com.dts.freefiremax` when the FF MAX tab is selected, while FF NORMAL continues targeting `com.dts.freefireth`.
